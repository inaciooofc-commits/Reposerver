...
def create_character(name, data=None):
    ...
    cur.execute('INSERT OR REPLACE INTO characters (name, data, created_at) VALUES (?, ?, ?)', (name, payload, datetime.now(timezone.utc).isoformat()))
    conn.commit()
    conn.close()
...
def save_action(who, action, result):
    ...
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute('INSERT INTO actions (who, action, result, ts) VALUES (?, ?, ?, ?)', (who, action, json.dumps(result), datetime.now(timezone.utc).isoformat()))
    conn.commit()
    conn.close()
